...
var copyLinkBtns = document.querySelectorAll('[data-copy-link-btn]');
...
var wordsFromDictionary = document.querySelectorAll('[data-dictionary]');
wordsFromDictionary.forEach(function (word) {
  word.textContent = dictionary[word.dataset.dictionary];
}); // Share documents

var quote = document.querySelector(".quote");
var quoteDocumentsBtn = document.querySelector('.quote__documents-btn');
var quoteDocumentsList = document.querySelector('.quote__documents-list');
var quoteDocumentsListBtnPng = document.querySelector('.quote__documents-list-btn--png');
var quoteDocumentsListBtnPdf = document.querySelector('.quote__documents-list-btn--pdf');
var quoteDocumentsListBtnXlsx = document.querySelector('.quote__documents-list-btn--xlsx');
var quoteCanvasData = {};
quoteDocumentsBtn.addEventListener('click', function () {
...
quoteDocumentsListBtnPng.addEventListener('click', function () {
  quoteDocumentsList.classList.remove('quote__documents-list_active');
});
quoteDocumentsListBtnPdf.addEventListener('click', function () {
  quoteDocumentsList.classList.remove('quote__documents-list_active');
  ...
  var docWidth = ConvertPxToMM(quoteCanvasData.width);
  var docHeight = ConvertPxToMM(quoteCanvasData.height);
  var orientation = quoteCanvasData.width >= quoteCanvasData.height ? 'l' : 'p';
  var doc = new jsPDF(orientation, 'mm', [docWidth, docHeight]);
  doc.addImage(quoteDocumentsListBtnPng.href, 0, 0);
  doc.save("VKCO.pdf");
});
quoteDocumentsListBtnXlsx.addEventListener('click', function () {
  ...
  var ws_data = widgetLanguage === 'ru' ? [['Дата', 'Цена', 'Объем']] : [['Date', 'Price', 'Turnover']];
  currentLabels.forEach(function (item, i) {
    var newRow = [];
...
var quoteWrapper = document.querySelector('.quote__wrapper');
var quotePeriodItem = document.querySelectorAll('.quote__period-item');
var quoteGraph = document.querySelector('.quote__graph');
var quoteGraphBar = document.querySelector('.quote__graph-bar');
var quoteGraphNavigationRangeButton = document.querySelectorAll('.quote__graph-navigation-range-button');
var quoteGraphNavigationRangeButtonMax = document.querySelector('.quote__graph-navigation-range-button--max');
var quoteGraphNavigationRangeButtonMin = document.querySelector('.quote__graph-navigation-range-button--min');
var quoteGraphNavigationRange = document.querySelector('.quote__graph-navigation-range');
var quoteGraphNavigationRangeLineMin = document.querySelector('.quote__graph-navigation-range-line--min');
var quoteGraphNavigationRangeLineMax = document.querySelector('.quote__graph-navigation-range-line--max');
var quoteGraphNavigationLabels = document.querySelector('.quote__graph-navigation-labels');
var quoteGraphNavigationScroll = document.querySelector('.quote__graph-navigation-scroll');
var quoteGraphNavigationControlButton = document.querySelectorAll('[data-navigation-control]');
var quoteGraphNavigationScrollOverlay = document.querySelector('.quote__graph-navigation-scroll-overlay');
var quoteGraphNavigationScrollButtonLess = document.querySelector('.quote__graph-navigation-scroll-button--less');
var quoteGraphNavigationScrollButtonMore = document.querySelector('.quote__graph-navigation-scroll-button--more');
var quoteGraphCurrentLabels = document.querySelector('.quote__graph-current-labels');
var quoteGraphLineTicks = document.querySelector('.quote__graph-ticks--line');
var quoteGraphBarTicks = document.querySelector('.quote__graph-ticks--bar');
var quoteGraphTicks = document.querySelectorAll('.quote__graph-ticks');
var quoteGraphWrapper = document.querySelectorAll('.quote__graph-wrapper');
var labels = [];
var values = [];
var volume = [];
var currentLabels = [];
...
var barChart;
var navigationChart;
var isRepeatedHistoryRequest = false;
urls = {
...
function getHeaderData(url) {
  var dataItems = document.querySelectorAll('[data-variable]');
  fetch(url).then(function (response) {
    return response.json();
  }).then(function (data) {
    var marketData = data.marketdata;
    dataItems.forEach(function (dataItem) {
      var dataItemVariable = dataItem.dataset.variable;

      if (marketData.data[0]) {
        marketData.columns.forEach(function (marketVariable, i) {
...
setInterval(function () {
  getHeaderData(urls.headerData);
}, 5000); // Get, update data for charts

function getHistoryData(_x, _x2) {
  return _getHistoryData.apply(this, arguments);
}

function _getHistoryData() {
  _getHistoryData = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(url, type) {
    var date,
        fullUrl,
        _args2 = arguments;
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            date = _args2.length > 2 && _args2[2] !== undefined ? _args2[2] : '';
            fullUrl = url + date;
            _context2.next = 4;
            return fetch(fullUrl).then(function (response) {
              ...
              if (type === 'get') {
                newData.history.data.forEach(function (array) {
                  labels.push(array[1]);
                  values.push(array[9]);
                  volume.push(array[4]);
                });
              ...
              if (type === 'update') {
                ...
                if (url === urls.historyData) {
                  ...
                  if (newData.history.data.length === 100) {
                    var _lastDate = currentLabels[currentLabels.length - 1];
                    getHistoryData(urls.historyData, 'update', _lastDate);
                ...
                if (url === urls.dailyData) {
                  newData.candles.data.forEach(function (array, i) {
...
function createGraph(type, labels, values, volume) {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            external: function external(context) {
              ...
              var tooltipModelValue = '';
              var tooltipModelVolume = '';
              currentLabels.map(function (item, i) {
              ...
              if (tooltipModel.body) {
                tooltipEl.innerHTML = "\n                  <div style=\"background: #FFFFFF; padding: 10px 15px; border: 1px solid #050B15; font-family: var(--font-family-primary); font-size: 16px;\">\n                    <div>".concat(tooltipModel.title, "</div>\n                    <div style=\"margin: 15px 0 10px; white-space: nowrap;\">\n                      <span style=\"color: #0077FF; margin-right: 10px\">").concat(widgetLanguage === 'ru' ? 'Цена' : 'Price', "</span> <span style=\"font-weight: 500; font-size: 22px;\">").concat(tooltipModelValue, "</span>\n                    </div>\n                    <div style=\"white-space: nowrap;\">\n                      <span style=\"color: #FF3583; margin-right: 10px\">").concat(widgetLanguage === 'ru' ? 'Объем' : 'Turnover', "</span> <span style=\"font-weight: 500; font-size: 22px;\">").concat(tooltipModelVolume, "</span>\n                    </div>\n                  </div>\n                  ");
              }
  ...
  if (type === 'bar') {
    ...
    barChart = new Chart(document.getElementById('barChart'), _config);
  }
  ...
  if (type === 'navigation') {
    ...
    navigationChart = new Chart(document.getElementById('navigationChart'), _config2);
  }
}
...
function updateCurrentLabelsList(currentLabels) {
  ...
  var updatedIndexesList = [];
  currentLabels.forEach(function (item, i) {
    if (i <= 10) {
      var newIndex = Math.round((currentLabels.length - 1) * 10 * i / 100);
      var checkRepeatedIndex = true;
...
function updateTicks() {
  ...
  var currentLineTicks = lineChart.scales.yAxis.ticks;
  var currentBarTicks = barChart.scales.yAxis.ticks;
  currentLineTicks.forEach(function (item) {
    var newTick = document.createElement('li');
    newTick.textContent = item.value;
    newTick.classList.add('quote__graph-tick');
    quoteGraphLineTicks.append(newTick);
  });
  currentBarTicks.forEach(function (item) {
    var newTick = document.createElement('li');
    newTick.textContent = item.value;
    newTick.classList.add('quote__graph-tick');
...
function updateSizes() {
  var ticksWidth = quoteGraphLineTicks.clientWidth > quoteGraphBarTicks.clientWidth ? quoteGraphLineTicks.clientWidth : quoteGraphBarTicks.clientWidth;
  quoteGraphTicks.forEach(function (item) {
    item.style.minWidth = ticksWidth + 'px';
  });
  var graphsWidth = quoteWrapper.clientWidth - ticksWidth;
  quoteGraphWrapper.forEach(function (wrapper) {
...
function firstRender() {
  return _firstRender.apply(this, arguments);
}

function _firstRender() {
  _firstRender = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee3() {
    return regeneratorRuntime.wrap(function _callee3$(_context3) {
      while (1) {
        switch (_context3.prev = _context3.next) {
          case 0:
            _context3.next = 2;
            return getHistoryData(urls.historyData, 'get');
...
firstRender(); // Changing the period on the charts

quotePeriodItem.forEach(function (item) {
  item.addEventListener('click', /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
    var now, nowTimestamp, date, dateTimestamp, period, url, fullDate, formatedFullDate;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            item.classList.add('quote__period-item_active');
            quotePeriodItem.forEach(function (otherItem) {
              if (otherItem != item) {
            ...
            date = '';
            _context.t0 = item.dataset.period;
            _context.next = _context.t0 === "all" ? 8 : _context.t0 === "year" ? 11 : _context.t0 === "month" ? 14 : _context.t0 === "week" ? 17 : _context.t0 === "day" ? 20 : 23;
            break;
          ...
          case 23:
            if (period) {
              ...
              fullDate = new Date(dateTimestamp);
              formatedFullDate = fullDate.getFullYear() + '-' + ('0' + (fullDate.getMonth() + 1)).slice(-2) + '-' + ('0' + fullDate.getDate()).slice(-2);
              date = formatedFullDate;
            }

            _context.next = 26;
            return getHistoryData(url, 'update', date);
...
function initNavigation() {
  var historyLength = labels.length;
  var firstDataIndex = 0;
  var lastDataIndex = historyLength - 1;
  var updatedLabels;
  var updatedValues;
  var updatedVolume;
  var navigationLabels = [];
  labels.forEach(function (item, i) {
    if (i <= 10) {
      var newIndex = Math.ceil((labels.length - 1) * 10 * i / 100);
      navigationLabels.push(labels[newIndex]);
    }
  });
  navigationLabels.forEach(function (item) {
    var newLabel = document.createElement('li');
  ...
  function updateGraphs(firstDataIndex, lastDataIndex) {
    updatedLabels = labels.filter(function (item, i) {
      return i >= firstDataIndex && i <= lastDataIndex;
    });
    updatedVolume = volume.filter(function (item, i) {
      return i >= firstDataIndex && i <= lastDataIndex;
    });
    updatedValues = values.filter(function (item, i) {
      return i >= firstDataIndex && i <= lastDataIndex;
    });
    updateGraph(updatedLabels, updatedValues, updatedVolume);
  }

  quoteGraphNavigationControlButton.forEach(function (btn) {
    btn.addEventListener('mousedown', function (event) {
      var shiftX = quoteGraphNavigationRange.getBoundingClientRect().left;
      var clickPositon = event.pageX - shiftX - btn.offsetLeft;
      var positionButtonMin = quoteGraphNavigationRangeButtonMin.offsetLeft;
      var positionButtonMax = quoteGraphNavigationRangeButtonMax.offsetLeft;
  ...
  quoteGraphNavigationScrollButtonLess.addEventListener('click', function () {
  ...
  quoteGraphNavigationScrollButtonMore.addEventListener('click', function () {
...
function updateScrollSize() {
  var scrollWidth = quoteGraphNavigationRangeButtonMax.offsetLeft - quoteGraphNavigationRangeButtonMin.offsetLeft;
...
function updateNavigation() {
  ...
  0;
  var firstLabel = currentLabels[0].split(' ')[0];
  var newStartIndex = labels.indexOf(firstLabel);

  if (newStartIndex >= 0) {
    quoteGraphNavigationRangeButtonMin.style.left = "".concat(newStartIndex * 100 / labels.length, "%");
    quoteGraphNavigationRangeLineMin.style.width = quoteGraphNavigationRangeButtonMin.offsetLeft + 'px';
  }
